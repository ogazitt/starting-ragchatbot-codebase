    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        for doc, meta in zip(results.documents, results.metadata):
            ...
            if lesson_num is not None:
                ...
                # Fetch the lesson link from vector store
                lesson_link = self.store.get_lesson_link(course_title, lesson_num)

            # Build source object with text and optional URL
            source_obj = {
                "text": source_text,
                "url": lesson_link
            }
...
class CourseOutlineTool(Tool):
    ...
    def get_tool_definition(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": "Get the complete outline of a course including its title, link, and all lessons with their numbers and titles",
            "input_schema": {
                "type": "object",
                "properties": {
                    "course_title": {
    ...
    def execute(self, course_title: str) -> str:
        ...
        try:
            ...
            course_title_full = metadata.get('title', 'unknown')
            course_link = metadata.get('course_link')
            lessons_json = metadata.get('lessons_json')

            if not lessons_json:
                return f"No lessons found for course '{course_title_full}'"

            lessons = json.loads(lessons_json)

            # Format the outline
            outline = f"Course: {course_title_full}\n"
            if course_link:
